...
export class ShooterPlayer extends Player {
    constructor(x, y, size, color, p, id) {
        ...
        this.angle = 0;  
        this.radius = 40;  
        this.angularVelocity = 1;  // MCU (Movimiento Circular Uniforme)
        this.maxAngularAcceleration = 5;  //MCUV (Movimiento Circular Uniforme Variado)
        this.isAlive = true;  
        this.id = id;  
    }
    ...
    update() {
        if (this.isAlive) {
            this.angle += this.angularVelocity;  // Incremento de velocidad
            if (this.angle >= 360) {
                this.angle = 0;
            }
            ...
            if (this.angularVelocity < this.maxAngularAcceleration) {
    ...
    hit() {
        this.isAlive = false;  // El jugador muere al ser impactado
    }
}
...
class Bullet {
    constructor(x, y, size, color, velocityX, velocityY, p, ownerId) {
        ...
        this.ownerId = ownerId;  // ID del jugador que disparó la bala
    }
